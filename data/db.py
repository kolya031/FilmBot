...
try:
    data=['https://ww5.frkp.lol', 'frkp', True, '▶️ Смотреть #1']
    cs.execute("INSERT INTO player_data VALUES(?, ?, ?, ?)", data)
    sql.commit()
except:
    pass
try:
    data=['www.ggkinopoisk.ru', 'vavada', False, '▶️ Смотреть #2']
    cs.execute("INSERT INTO player_data VALUES(?, ?, ?, ?)", data)
    sql.commit()
except:
    pass
...
try:
    data=['wellcome', '*🙋‍♂️ Привет* [{full_name}](tg://user?id={user_id}), *ты в {username_bot}. Данный бот предназначен для поиска фильмов по КОДУ. *']
