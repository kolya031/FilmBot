...
async def myling_list_start_admin(call: types.Message, state: FSMContext):
    message_data=await bot.send_message(chat_id=call.from_user.id, text='Хорошо, отправьте текст для рассылки. ✒️\nМожно использовать стандартную разметку. ✂️', reply_markup=ikb_back)
...
async def add_film_admin(call: types.Message, state: FSMContext):
    message_data=await bot.send_message(chat_id=call.from_user.id, text='🔑 Хорошо, отправьте мне код.', reply_markup=types.InlineKeyboardMarkup(row_width=1).\
        add(types.InlineKeyboardButton(text='♻️ Сгенирировать', callback_data='generetion_fims_code_admin'), ikb_back_oikb))
    async with state.proxy() as data:
        data['message_id']=message_data.message_id
...
async def add_film_generetion_fims_code(call: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        data['code']=code
    await call.message.edit_text('🎫 Хорошо, теперь отправьте мне название фильма.')
...
async def delete_film_admin(message: types.Message, state: FSMContext):
    message_data=await bot.send_message(chat_id=message.from_user.id, text='🗑 Хорошо, отправьте код фильма которого желаете удалить.', reply_markup=ikb_back)
    async with state.proxy() as data:
        data['message_id']=message_data.message_id
...
async def add_chennel_admin(message: types.Message, state: FSMContext):
    message_data=await bot.send_message(chat_id=message.from_user.id, text='Хорошо, дайте в канале права "просматривать участников" и "пригласительные ссылки", после, отправьте мне @username или id канала которого желаете добавить.', reply_markup=ikb_back)
    async with state.proxy() as data:
        data['message_id']=message_data.message_id
...
async def add_chennel_admin(message: types.Message, state: FSMContext):
    message_data=await bot.send_message(chat_id=message.from_user.id, text='Хорошо, какой канал желаете удалить?', reply_markup=ikb_back)
    async with state.proxy() as data:
        data['message_id']=message_data.message_id
...
async def check_chennel_admin(message: types.Message, state: FSMContext):
    text=''
    message_data=await bot.send_message(chat_id=message.from_user.id, text='Хорошо, идёт проверка, ожидайте.')
    me=await bot.get_me()
            ...
            for e in me_chat_status:
                if e['user']['username'] == me_username:
                    if e['can_invite_users']:
                        text+=f'Канал: {i[1]} прошёл проверку.\n\n'
                    else:
                        text+=f'Канал: {i[1]} не имеет доступа к ссылкам.\n\n'
                    await bot.edit_message_text(chat_id=message.from_user.id, message_id=message_data.message_id, text=f'Хорошо, идёт проверка, ожидайте.\n\n{text}')
                    break
        except:
            await delete_Chennel(chennel_identifier=i[0])
            text+=f'Был удален {i[1]}🗑\n\n'
            await bot.edit_message_text(chat_id=message.from_user.id, message_id=message_data.message_id, text=f'Хорошо, идёт проверка, ожидайте.\n\n{text}')
    await bot.edit_message_text(chat_id=message.from_user.id, message_id=message_data.message_id, text=f'Проверка закончена.\n\n{text}\n\nТак же, если изменились названия каналов, то в кнопке они поменяются.', reply_markup=ikb_close)
...
async def chennger_kbname_player_admin(call: types.CallbackQuery, state: FSMContext):
    message_data1=await bot.send_message(chat_id=call.from_user.id, text='📌 Хорошо, отправьте новое название кнопки.', reply_markup=ikb_back)
...
async def chennger_wellcome_text_settings_admin(call: types.CallbackQuery, state: FSMContext):
    message_data=await bot.send_message(chat_id=call.from_user.id, text='{username_bot}-username бота\n{bot_id}-id бота\n{username}-username пользователя\n{full_name}-полное имя пользователя\n{user_id}-id пользователя\n{film_name}-название фильма\n{film_code}-код от фильма\n\nМожно ипользовать разметку HTML✂️\n\nХорошо, отправьте новый текст для фильмов.', reply_markup=ikb_back)
    async with state.proxy() as data:
        data['message_id']=message_data.message_id
