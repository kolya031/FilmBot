...
async def myling_list(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        for i in data_user:
            try:
                await bot.copy_message(from_chat_id=message.from_user.id, chat_id=i, message_id=msg_myling, parse_mode=types.ParseMode.MARKDOWN)
                count_accept+=1
                await bot.edit_message_text(chat_id=message.from_user.id, message_id=data['message_id'], text=f'*Данные о рассылки\nТекст: "{message.text}"\n✅Успешно: {count_accept}\n❌Ошибки: {count_error}*', parse_mode=types.ParseMode.MARKDOWN)
            except:
                count_error+=1
...
async def state_add_film_code(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        await bot.edit_message_text(chat_id=message.from_user.id, message_id=data['message_id'], text='Хорошо, теперь отправьте название фильма.', reply_markup=ikb_back)
...
async def state_add_film_name(message: types.Message, state: FSMContext):
    ...
    await bot.edit_message_text(chat_id=message.from_user.id, message_id=data['message_id'], text='Отлично, теперь отправьте фотографию для обложки.')
...
async def state_add_film_priew(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        try:
            ...
            await message.answer('Скорее всего данный код уже был добавлен.')
    await state.finish()
...
async def state_add_film_priew_no_photo(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        try:
            await bot.edit_message_text(chat_id=message.from_user.id, message_id=data['message_id'], text='Отрпавьте фото.\nОтлично, теперь отправьте фотографию для обложки.', reply_markup=ikb_back)
        except:
            pass
...
async def delete_film(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        if await delete_Film(code=chennel_identifier):
            await bot.edit_message_text(chat_id=message.from_user.id, message_id=data['message_id'], text='❎ Удалено.', reply_markup=ikb_close)
            await state.finish()
        else:
            try:
...
async def delete_chennel(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        if await delete_Chennel(chennel_identifier=message.text):
            await bot.edit_message_text(chat_id=message.from_user.id, message_id=data['message_id'], text='Канал удалён.', reply_markup=ikb_close)
            await state.finish()
        else:
            await bot.edit_message_text(chat_id=message.from_user.id, message_id=data['message_id'], text='Извините, вы не добавляли данный канал.', reply_markup=ikb_back)
...
async def chennger_kbname_player(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        await bot.edit_message_text(chat_id=message.from_user.id, message_id=data['message_id1'], text='Кнопка успешно изменена.', reply_markup=ikb_close)
...
async def chennger_wellcome_text(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        try:
            ...
            await bot.edit_message_text(chat_id=message.from_user.id, message_id=data['message_id'], text='Успешно изминили приветсвие.', reply_markup=ikb_close)
            await state.finish()
...
async def chennger_wellcome_text(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        try:
            await bot.edit_message_text(chat_id=message.from_user.id, message_id=data['message_id'], text=message.text, parse_mode=types.ParseMode.MARKDOWN)
            await bot.edit_message_text(chat_id=message.from_user.id, message_id=data['message_id'], text='✅ Успешно изминили название фильма.', reply_markup=ikb_close)
